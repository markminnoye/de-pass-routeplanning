...
|---|---|---|
| Scenario bedenken of aanpassen | scenario-JSON (`references/data-schema.md`) | nee |
...
Aanroep, vanuit de werkmap waar `busroutes/` staat: `python3 -m busroutes.cli <commando>`. Een losse reistijd is een scenario van die rit, daarna `evaluate --offline`.
...
1. `python3 -m busroutes.cli data status`
2. Schrijf `scenarios/<naam>.json`. Schema: `references/data-schema.md`. Elke leerling exact één keer.
...
De plugin bevat geen datapakket. De school zet haar map (school, leerlingen, bussen, opstapplaatsen, `matrix/`) buiten de plugin en wijst die aan met `BUSROUTES_DATA_DIR` of `--data`. Zonder die instelling is de default `docs/samples/`. De fictieve set in de git-repo is alleen het voorbeeld; die zit niet in de plugin.
